...
def validate_environment(api_key,litellm_params):
    headers = {
...
def completion(
    ...
    headers = validate_environment(api_key,litellm_params)
    model = model
    ...
    ## COMPLETION CALL
    modelUri = f"gpt://{litellm_params['YANDEX_FOLDER_ID']}/{model}"  
    messages_without_empty = []
    for message in messages:
        if "content" in message:
            if message["content"] == "":
                continue
            message["text"] = message.pop("content")
            messages_without_empty.append(message)        
    ...
    data = {
        ...
        "messages": messages_without_empty
    }
    ...
    if False:
        ...
        if "error" in completion_response:
            ...
            try:
                model_response["choices"][0]["message"]["content"] = (
                    completion_response["result"]["alternatives"][0]["message"]["text"]
                )
            except:
                raise YandexGPTError(
        ...
        usage = Usage(
            prompt_tokens=completion_response["result"]["usage"]["inputTextTokens"],
            completion_tokens=completion_response["result"]["usage"]["completionTokens"],
            total_tokens=completion_response["result"]["usage"]["totalTokens"],
        )
...
def embedding(
    ...
    if not response.ok:
        raise YandexGPTError(message=response.text, status_code=response.status_code)
    completion_response = response.json()
